...
export default function SideBarItems({sidebarItems,activeSubmenu,submenuVariants,isDesktop ,toggleSubmenu}){
    ...
    return (
        <nav className="p-4 overflow-hidden h-screen">
        {sidebarItems.map((item, index) => (
          <div key={index}>
            <div
              className={`flex items-center p-3 mb-2 rounded-lg cursor-pointer transition-colors ${location.pathname === item.path
                ...
                {item.submenu &&
                  item.submenu.map((subItem, subIndex) => (
                    <Link to={subItem.path} key={subIndex}>
                      <div
                        className={`flex items-center p-2 mb-1 rounded-lg cursor-pointer transition-colors ${location.pathname === subItem.path
