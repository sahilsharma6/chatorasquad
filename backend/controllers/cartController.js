    ...
    try {
        ...
        if (!cart) {
                ...
                items: [
                    {
                        ...
                        name: menuItem.name,
                        quantity: 1,
                        price: menuItem.discountedPrice || menuItem.sellingPrice,
                    },
                ],
                total: menuItem.discountedPriceprice || menuItem.sellingPrice,
            });
            await newCart.save();
            ...
            if (existingItem) {
                ...
                const price= menuItem.discountedPrice || menuItem.sellingPrice ;
                cart.total += price;
            } else {
                cart.items.push({
                    itemId,
                    name: menuItem.name,
                    quantity:1,
                    price: menuItem.discountedPriceprice || menuItem.sellingPrice,
                });
                cart.total += menuItem.discountedPriceprice || menuItem.sellingPrice;
            }
...
export const updateQuantity = async (req, res) => { 
    try{
        ...
        cart.total += (quantity - prevQuantity) * item.price;
        await cart.save();
